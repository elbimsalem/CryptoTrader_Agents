...
class TestModeCryptoTradingCrew():
    ...
    def __init__(self, initial_balance: float = 10000.0, verbose: bool = False, retry_config: Dict[str, Any] = None):
        ...
        # Initialize base autonomous crew with paper trading enabled and portfolio simulator
    ...
    def virtual_trade_executor(self) -> Agent:
        """Virtual Trade Executor for Test Mode (replaces real trade executor)"""
        return Agent(
            config=self.test_agents_data.get('virtual_trade_executor', {
            ...
            tools=[self.tool_manager.get_portfolio_simulator_tool()],  # Use portfolio simulator instead of Binance API
            llm=get_test_mode_llm(),
            verbose=self.verbose
        )
    ...
    def execute_virtual_trading_strategy_task(self) -> Task:
        """Execute virtual trading strategy using portfolio simulator"""
        return Task(
            config=self.test_tasks_data.get('execute_virtual_trading_strategy', {
                'description': 'Execute trading strategy using virtual portfolio simulation',
                'expected_output': 'Virtual trade execution results'
            }),
            agent=self.virtual_trade_executor(),
            context=[
    ...
    def crew(self) -> Crew:
        ...
        try:
            # Get all base crew agents (trade_executor will automatically be virtual due to paper_trading=True)
            base_agents = [
                ...
                self.base_crew.trade_executor(),  # This will be virtual trade executor automatically
            ...
            base_tasks = [
                ...
                self.base_crew.assess_portfolio_risk_task(),
                self.base_crew.optimize_portfolio_allocation_task(),
                self.base_crew.execute_trading_strategy_task(),  # This will use virtual executor automatically
